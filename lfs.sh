note:this bash script has not been tested by the author and this script is not an LFS system installer. This is just an approximate list of commands for installing this system

#cfdisk interface. creating partitions
...
# Part 2: Downloading and installing the basic LFS system

#installing the necessary packages in a Live CD (example for SystemRescue):
pacman -Sy wget gcc make glibc bison texinfo

#creating a source directory and downloading the necessary packages:
...
#creating an lfs user:
groupadd lfs
useradd -s /bin/bash -g lfs -m -k /dev/null lfs
passwd lfs  # Set the password for the lfs user

chown -v lfs /mnt/{usr{,/*},lib,var,etc,bin,sbin,tools}
case $(uname -m) in
  x86_64) chown -v lfs /mnt/lib64 ;;
esac

chown -v lfs /mnt/sources

#setting up an environment for an lfs user:
su - lfs
